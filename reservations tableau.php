...
<body>
  ...
  <nav class="bg-black">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      ...
      <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1">
        <ul class="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-black">
  ...
  <section class="relative">
    <div class="p-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div class="col-span-1 lg:col-span-2 bg-gray-300 p-6 rounded-lg shadow-md">
        ...
        <div class="overflow-x-auto">
          <table class="w-full border-collapse border border-gray-400">
            ...
            <tbody>
              <?php
                include ("db.php");
                $resv = "SELECT ID_reservation, clients.nom, clients.prenom, activites.titre, reservations.date_reservation, reservations.status FROM reservations INNER JOIN clients ON reservations.ID_client = clients.ID_client INNER JOIN activites ON reservations.ID_activite = activites.ID_activite";
                $res = mysqli_query($conn, $resv);
                while ($row = mysqli_fetch_assoc($res)) {
                  echo "<tr class='hover:bg-blue-500'>
                          <td class='border border-gray-400 px-4 py-2'>{$row['nom']}</td>
                          <td class='border border-gray-400 px-4 py-2'>{$row['titre']}</td>
                          <td class='border border-gray-400 px-4 py-2'>{$row['date_reservation']}</td>
                          <td class='border border-gray-400 px-4 py-2'>{$row['status']}</td>
                        </tr>";
                }
              ?>
            </tbody>
